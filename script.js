(function () {
  ...
  const tagPanelToggle = document.querySelector(".tag-panel-toggle");
  const tagPanelBody = document.querySelector("#tag-panel-body");
  ...
  if (archiveFilter && selectedFilters && archiveEpisodes.length > 0) {
    ...
    if (tagPanelToggle && tagPanelBody && tagSearchInput && tagSuggestions) {
      tagPanelToggle.addEventListener("click", function () {
        const isOpen = tagPanelBody.hidden;
        tagPanelBody.hidden = !isOpen;
        tagPanelToggle.setAttribute("aria-expanded", String(isOpen));
        if (isOpen) {
